...
export default function DomainSection({
  ...
  const [displayValue, setDisplayValue] = useState<string>("papermark.xeedgroup.co");
  ...
  const isEditingCustomDomain =
    editLink && data.domain && data.domain !== "papermark.xeedgroup.co" ? true : false;
  ...
  const handleDomainChange = (value: string) => {
    ...
    if (value === "add_domain" || value === "add_dataroom_domain") {
      setModalOpen(true);
      setData({ ...data, domain: "papermark.xeedgroup.co" });
      setDisplayValue("papermark.xeedgroup.co");
      return;
    }

    // Check if this is a custom domain selection (not papermark.xeedgroup.co)
    if (value !== "papermark.xeedgroup.co") {
  ...
  useEffect(() => {
    if (domains && !editLink) {
      ...
      const domainValue = canUseCustomDomain
        ? (defaultDomain?.slug ?? "papermark.xeedgroup.co")
        : "papermark.xeedgroup.co";
  ...
  const defaultDomain = editLink
    ? (data.domain ?? "papermark.xeedgroup.co")
    : (linkType === "DOCUMENT_LINK" && canUseCustomDomainForDocument) ||
        (linkType === "DATAROOM_LINK" && canUseCustomDomainForDataroom)
      ? (domains?.find((domain) => domain.isDefault)?.slug ?? "papermark.xeedgroup.co")
      : "papermark.xeedgroup.co";
  ...
  return (
    <>
        ...
        >
          <SelectTrigger
            className={cn(
              ...
              data.domain && data.domain !== "papermark.xeedgroup.co"
                ? ""
          ...
          <SelectContent className="flex w-full rounded-md border border-input bg-white text-foreground placeholder-muted-foreground focus:border-muted-foreground focus:outline-none focus:ring-inset focus:ring-muted-foreground dark:border-gray-500 dark:bg-gray-800 focus:dark:bg-transparent sm:text-sm">
            <SelectItem value="papermark.xeedgroup.co" className="hover:bg-muted">
              papermark.xeedgroup.co
            </SelectItem>
            {linkType === "DOCUMENT_LINK" && (
              <>
        ...
        {data.domain && data.domain !== "papermark.xeedgroup.co" ? (
          <Input
            ...
            className={cn(
              ...
              data.domain && data.domain !== "papermark.xeedgroup.co" ? "flex" : "",
      ...
      {data.domain && data.domain !== "papermark.xeedgroup.co" && !isDomainVerified ? (
